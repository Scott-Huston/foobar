...
hours-long lines, so the checkpoint guards have found ways to quicken the pass-through 
rate. Instead of checking each and every worker coming through, the guards instead 
go over everyone in line while noting their security IDs, then allow the line to 
fill back up. Once they've done that they go over the line again, this time leaving 
off the last worker. They continue doing this, leaving off one more worker from the 
line each time but recording the security IDs of those they do check, until they skip 
the entire line, at which point they XOR the IDs of all the workers they noted into 
a checksum and then take off for lunch. Fortunately, the workers' orderly nature 
...
With this information, write a function solution(start, length) that will cover for 
...
automatic review occurs, so your program must generate the proper checksum with just 
those two values.
"""

def solution(start, length):
    def xor_zero_to_num(n):
        """
        Finds the xor of all numbers from 0 to n
        """
        xors = [n,1,n+1,0]
        return xors[n%4]

    def xor_of_range(a,b):
        """
        Finds the xor of range from a to b inclusive because 
        the xor of the repeated values from 0 to a cancel out
        """
        return xor_zero_to_num(a-1)^xor_zero_to_num(b)
    ...
    while (length-count)>0:
        solution ^= xor_of_range(start, start+length-count-1)
        start += length
        count += 1
